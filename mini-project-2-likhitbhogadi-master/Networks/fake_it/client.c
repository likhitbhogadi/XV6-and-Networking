...
int sending_mode = 1; // if 1 it will send else receive for 0
...
int receive_data(int sockfd, struct sockaddr_in *client_addr, socklen_t addr_len) {
    ...
    if (FD_ISSET(sockfd, &readfds)) {
        if (recvfrom(sockfd, &pkt, sizeof(pkt), 0, (struct sockaddr *)client_addr, &addr_len) > 0) {
            ...
            // if(count % 3 != 1 ){ // dont send sometimes for checking retransmission
...
int main() {
    ...
    // it needs the servers address to send the data, ip is in #define
    ...
    while(1){
        if(sending_mode){
            ...
            send_data(sockfd, &server_addr, addr_len, buffer);
            sending_mode = 0;
            // sleep(2);  // Sleep for 10 milliseconds (0.1 seconds)
        }
        else{
            printf("\nServer is waiting for data...\n\n");
            while( receive_data(sockfd, &server_addr, addr_len) == 0 ){
                // do ntg;
            }
            sending_mode = 1;
        }
