...
struct proc
{
  ...
  // Array to keep count of system calls
  int syscall_counts[32]; // Supports up to 31 system calls 
  // comes under which category?? , we have 3 categories above
  ...
  int time_slice;         // Remaining time slice for current priority
  int total_ticks;        // Total ticks the process has run
};
